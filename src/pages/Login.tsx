      ...
      <div className="app-container py-8 sm:py-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
          ...
          <div className="p-5 pb-3">
            <button
              ...
              disabled={busy}
              className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-orange-400 to-orange-500 text-white font-semibold h-12 rounded-xl shadow-md shadow-orange-200 hover:from-orange-500 hover:to-orange-600 active:scale-95 transition-transform disabled:opacity-50 text-base"
            >
              {busy ? (
            ...
            <div className="flex bg-orange-50 rounded-xl p-1">
              {([
                ...
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 py-3 min-h-[44px] rounded-lg text-xs sm:text-sm font-medium active:scale-95 transition-transform ${
                    mode === item.key
          ...
          <form onSubmit={handleSubmit} className="p-5 pt-4 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  autoComplete="email"
                  className="w-full pl-10 pr-4 h-12 border border-gray-200 rounded-xl text-base focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all"
                />
              </div>
            </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  {mode === 'login' && (
                    <button
                      ...
                      disabled={busy}
                      className="text-xs text-orange-500 hover:text-orange-600 underline underline-offset-2 active:scale-95 transition-transform"
                    >
                      忘记密码？
                    </button>
                  )}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 h-12 border border-gray-200 rounded-xl text-base focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all"
                  />
                  <button
                    type="button"
            ...
            <button
              type="submit"
              disabled={busy}
              className="w-full flex items-center justify-center gap-2 bg-orange-500 text-white font-medium h-12 rounded-xl hover:bg-orange-600 active:scale-95 transition-transform disabled:opacity-50 text-base"
            >
              {busy ? (
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : mode === 'magiclink' ? (
        ...
        <div className="mt-4 bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-orange-50/50 transition-colors active:scale-95 transition-transform"
          >
        ...
        <div className="mt-4 bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600">连接诊断</span>
            </div>
            {diagExpanded ? (
              <ChevronUp className="w-4 h-4 text-gray-400 flex-shrink-0" />
            ) : (
          ...
          {diagExpanded && (
            <div className="px-4 pb-4 pt-1 border-t border-gray-50">
              <p className="text-xs text-gray-400 mb-3">检测 Supabase 服务连通性，排查网络问题</p>
              <button
                onClick={runDiagnostic}
                disabled={diagnosing}
                className="flex items-center gap-1.5 text-sm text-orange-500 hover:text-orange-600 transition-colors disabled:opacity-50 active:scale-95 transition-transform"
              >
                {diagnosing ? (
              ...
              {diagResults.length > 0 && (
                <div className="mt-3 bg-orange-50/50 rounded-xl overflow-hidden">
                  {diagResults.map((item) => (
                    <div
                      key={item.name}
                      className="flex items-center gap-3 px-3 py-2.5 border-b border-orange-100/50 last:border-b-0"
                    >
                      {item.status === 'success' ? (
